...
ProtoCalendar.withinViewport = function(element) {
  ...
  var offsets;
  ProtoCalendar.executeWithVisibility(element, function() { offsets = element.viewportOffset(); });
  var offsetX = offsets.left;
  var offsetY = offsets.top;
  return (offsetX + width <= document.viewport.getWidth()) &&
    (offsetY + height <= document.viewport.getHeight());
}

ProtoCalendar.executeWithVisibility = function(element, func) {
  element = $(element);
...
AbstractProtoCalendarRender.prototype = {
  ...
  setPosition: function() {
    ...
    this.setAlignment(this.alignTo, this.container);
    if (this.iframe) {
      ...
      this.setAlignment(this.alignTo, this.iframe);
    }
  },

  setAlignment: function(alignTo, element) {
  ...
  show: function(option) {
    ...
    this.scrollIfInvisible();
  },

  scrollIfInvisible: function() {
    var container = this.container;
    if (!ProtoCalendar.withinViewport(this.container)) {
      ProtoCalendar.executeWithVisibility(container, function() { container.scrollTo(); });
    }
  },    
